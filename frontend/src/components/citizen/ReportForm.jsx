...
const ReportForm = ({ isOpen, onClose }) => {
  ...
  async function getPresign(file) {
    // apiService will attach the Authorization header (interceptor) if token is in localStorage
    const response = await apiService.post("/uploads/presign", {
      fileName: file.name,
      fileType: file.type,
      fileSize: file.size,
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !description || !location || !photo) {
      setError(
        "Please fill all required fields, select a location, & upload an image."
      );
      return;
    }
    ...
    try {
      ...
      await uploadToS3(photoPresign.uploadUrl, photo, (p) =>
        setUploadProgress(p * 0.6)
      ); // photo takes 60% of progress
      ...
      if (audio) {
        ...
        const audioFile =
          audio instanceof File
            ? audio
            : new File([audio], `voice-${Date.now()}.webm`, {
                type: audio.type || "audio/webm",
              });
        const audioPresign = await getPresign(audioFile);
        await uploadToS3(audioPresign.uploadUrl, audioFile, (p) =>
          setUploadProgress(60 + Math.round(p * 0.4))
        ); // audio covers last 40%
      ...
      const confirmRes = await apiService.post("/uploads/confirm", {
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        {error && (
          <p className="text-red-500 text-sm bg-red-50 dark:bg-red-900/20 p-3 rounded-md">
            {error}
          </p>
        )}
        ...
        <div>
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="text-gray-500 dark:text-gray-400 self-center hidden sm:inline">
                or
              </span>
